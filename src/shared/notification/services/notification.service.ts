...
export class NotificationService {
    ...
    async sendNotificationToDevice(
        pushDto: SendNotificationDto,
    ): Promise<void> {
        // Dispatch Push Notification job to queue
        this.logger.verbose(
            `Dispatching Push notification job ${JSON.stringify(pushDto)}`,
        );
    ...
    async sendNotificationToTopic(
        pushDto: SendTopicNotificationDto,
    ): Promise<void> {
        // Dispatch Push notification job to queue
        this.logger.verbose(
            `Dispatching push notification job to topic ${JSON.stringify(
                pushDto,
            )}`,
        );
        const job = await this.topicNotificationQueue.add(
            PUSH_NOTIFICATION_TOPIC_JOB,
            pushDto,
        );
