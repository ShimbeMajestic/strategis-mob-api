...
export class NotificationConsumer {
    ...
    async processPushNotificationQueue(job: Job<SendNotificationDto>): Promise<void> {

        const data = Object.assign(new SendNotificationDto, job.data); // deserialize job data into Sendpush notificationDto class

        this.logger.verbose(`Processing push notification job ID:${job.id}, ${JSON.stringify(data)}`);

        await this.firebaseService.notifyCustomer(data)
            .catch(error => {
...
export class NotificationTopicConsumer {
    ...
    async processPushNotificationQueue(job: Job<SendTopicNotificationDto>): Promise<void> {

        const data = Object.assign(new SendTopicNotificationDto, job.data); // deserialize job data into Sendpush notificationDto class

        this.logger.verbose(`Processing push notification job ID:${job.id}, ${JSON.stringify(data)}`);

        await this.firebaseService.notifyCustomersTopic(data)
            .catch(error => {
                this.logger.error(error.message);
