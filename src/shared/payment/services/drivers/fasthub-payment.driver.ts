...
export class FasthubPayment implements PaymentDriver {
    ...
    async sendPush(paymentDto: PaymentDto): Promise<any> {
        ...
        const channelID = paymentDto.operator == 'TIGOPESA' || paymentDto.operator == 'AIRTELMONEY' ? paymentConfig.fastHub.channelID : paymentConfig.fastHub.altChannelID;
        const endpoint = paymentDto.operator == 'TIGOPESA' || paymentDto.operator == 'AIRTELMONEY' ? paymentConfig.fastHub.endpoint : paymentConfig.fastHub.altEndpoint;
        ...
        try {
            ...
            return { status: error.response.status, message: error.response.data, isSuccessful: false };
        }
    ...
    protected async hashRequest(paymentDto: PaymentDto, channelID: string): Promise<string> {
        const hashString = `amount=${paymentDto.amount.toFixed(1)}&channel=${parseInt(channelID)}&callback_url=${encodeURIComponent(paymentConfig.callback)}&recipient=${paymentDto.recipient}&reference_id=${paymentDto.referenceID}&trx_date=`;
        this.logger.debug(hashString)
        this.logger.debug(paymentConfig.fastHub.key)

        var CryptoJS = require("crypto-js");
        const hash = CryptoJS.HmacSHA256(hashString, paymentConfig.fastHub.key).toString();
        return hash;
    }
