...
export class PermissionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const permissionGrants = await PermissionService.userPermissionGrants(
            user.id,
        );

        // Check each permission is available on User
        for (const permission of permissions) {
            const isFound = !!permissionGrants?.find(
                (p) => p.name === permission,
            );
            ...
            if (!isFound)
                throw new ForbiddenError(
                    `Forbidden! Permission '${permission}' required to access this resource`,
                );
        }
